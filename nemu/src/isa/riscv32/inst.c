...
static void csr_write(uint32_t csr,word_t data){
  switch(csr){
    case MTVEC:   cpu.csr.mtvec = data;  break;
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  jal_ftrace(rd,s->pc,s->dnpc));
  INSTPAT("??????? ????? ????? 000 ????? 1100111", jalr   , I, R(rd) = s->pc + 4;s->dnpc=src1+imm;\
  jalr_ftrace(s->isa.inst,rd,imm,s->pc,s->dnpc));
  
  INSTPAT("??????? ????? ????? 000 ????? 1100011", beq    , B, s->dnpc=(src1==src2)?s->pc+imm:s->dnpc);
  INSTPAT("??????? ????? ????? 001 ????? 1100011", bne    , B, s->dnpc=(src1!=src2)?s->pc+imm:s->dnpc);
  INSTPAT("??????? ????? ????? 100 ????? 1100011", blt    , B, s->dnpc=((int32_t)src1<(int32_t)src2)?s->pc+imm:s->dnpc);
  INSTPAT("??????? ????? ????? 101 ????? 1100011", bge    , B, s->dnpc=((int32_t)src1>=(int32_t)src2)?s->pc+imm:s->dnpc);
  INSTPAT("??????? ????? ????? 110 ????? 1100011", bltu   , B, s->dnpc=((uint32_t)src1<(uint32_t)src2)?s->pc+imm:s->dnpc);
  INSTPAT("??????? ????? ????? 111 ????? 1100011", bgeu   , B, s->dnpc=((uint32_t)src1>=(uint32_t)src2)?s->pc+imm:s->dnpc);
  ...
  INSTPAT("??????? ????? ????? 001 ????? 1110011", csrrw  , I, R(rd) = csr_read(imm);csr_write(imm,src1);
  printf("写入imm:0x%08x data:0x%08x\n",imm,src1));
  INSTPAT("??????? ????? ????? 010 ????? 1110011", csrrs  , I, word_t t=csr_read(imm);
  csr_write(imm,t|src1); R(rd) = t;printf("读出imm:0x%08x data:0x%08x\n",imm,src1));//当rs1=x0，可以视为只读，相当与rd=csr

  INSTPAT("0000000 ????? ????? 001 ????? 0010011", slli   , I, R(rd) = src1 << (imm&31));
  INSTPAT("0000000 ????? ????? 101 ????? 0010011", srli   , I, R(rd) = src1 >> (imm&31));
  INSTPAT("0100000 ????? ????? 101 ????? 0010011", srai   , I, R(rd) = (int32_t)src1 >> (imm&31));
  ...
  INSTPAT("0100000 ????? ????? 101 ????? 0110011", sra    , R, R(rd) = (int32_t)src1 >> (int32_t)src2);
