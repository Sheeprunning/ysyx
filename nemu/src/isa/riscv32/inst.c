...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 100 ????? 1100011", blt    , B, s->dnpc=(src1<src2)?s->pc+imm:s->dnpc);
  INSTPAT("??????? ????? ????? 101 ????? 1100011", bge    , B, s->dnpc=(src1>=src2)?s->pc+imm:s->dnpc);
  ...
  INSTPAT("??????? ????? ????? 010 ????? 0010011", slti   , I, R(rd) = src1 < imm);
  ...
  INSTPAT("0000000 ????? ????? 101 ????? 0010011", srli   , I, R(rd) = (uint32_t)src1 >> (imm&31));
  INSTPAT("0100000 ????? ????? 101 ????? 0010011", srai   , I, R(rd) = src1 >> (imm&31));
  ...
  INSTPAT("0000010 ????? ????? 000 ????? 0110011", and    , R, R(rd) = src1 * src2);
