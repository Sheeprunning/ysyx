...
//state转移
  always@(*)begin
    ...
    case(state)
      CMD:next_state=(counter==5'd7)?ADDR:state;
      ADDR:next_state=(counter!=5'd13)?state:
                      (cmd==8'heb)?WAIT:
                      (cmd==8'h38)?WRITE:ERROR;
      WAIT:next_state=(counter==5'd19)?READ:state;
      READ:next_state=(counter==5'd27)?CMD:state;
      WRITE:next_state=(counter==5'd21)?CMD:state;
      default: begin
          state <= state;
...
//获取cmd（8个周期）
  always@(posedge sck or posedge rst)begin
    if(rst)cmd<=0;
    else if(state==CMD)cmd<={cmd[6:0],dio[0]};
  end
...
//获取wdata(根据ce_n传输)
  wire[1:0] byte_index_w = {counter - 8'd14}[2:1];
...
//数据输出
  wire[1:0] byte_index_r = {counter[4:1] - 8'd10}[1:0];//从EF_PSRAM_CTRL.v中学习
